...
function RenderHeading({ block }: { block: HeadingBlock }) {
  ...
  return (
    <Tag
      ...
      className={`${block.textAlign} ${block.fontSize} font-bold`}
    >
...
function RenderCountdown({ block }: { block: CountdownBlock }) {
  ...
  const alignClass =
    block.textAlign === "text-center" ? "items-center" : block.textAlign === "text-right" ? "items-end" : "items-start";
  ...
  return (
    <div className={`flex flex-col gap-2 ${alignClass}`}>
      ...
      {timeLeft.expired ? (
        <p style={{ color: block.color }} className={`text-sm font-medium ${block.textAlign}`}>
          Time's up!
        </p>
      ) : (
...
function RenderBlock({ block, settings }: { block: Block; settings: ProjectSettings }) {
  switch (block.type) {
    ...
    case "button":
      return <RenderButton block={block} settings={settings} />;
    case "divider":
...
export function LandingPageRenderer({ project, className }: LandingPageRendererProps) {
  ...
  return (
    <div className={className} style={{ backgroundColor: settings.backgroundColor }}>
